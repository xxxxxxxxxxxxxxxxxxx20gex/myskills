...
MODULES = [
    ("skills-showcase.html", "页面外壳", "保留语义化 HTML、动态卡片容器、AI 详情、文件管理和右侧对话区域"),
    ...
    ("playground/static/js/app.js", "前端编排", "读取实际 Skill 目录并组合目录、管理、对话与面板缩放模块"),
    ("playground/server.py", "本地 Runner", "注册原始 Skills，桥接 Codex CLI，提供运行与 AI 分析接口并执行静态资源白名单"),
    ("playground/skill_service.py", "Skill 管理服务", "扫描目录，校验导入，过滤敏感文件，按维护标签移动，导出、预览、回收删除并持久化 AI 结论"),
    ("playground/run_registry.py", "运行记录", "以线程安全快照保存有限数量的运行状态、日志和产物元数据"),
    ...
    ("playground/tests/test_run_registry.py", "后端回归测试", "验证运行记录隔离、清理和活动任务保留逻辑"),
    ("playground/tests/test_skill_service.py", "Skill 管理回归测试", "验证导入、脱敏导出、敏感预览拦截和可恢复删除"),
...
def build_document(root: Path, config: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        *module_rows,
        ...
        "| `/api/skills/tree?path=...` | GET | 返回单个 Skill 的安全目录树 |",
        "| `/api/skills/file?path=...&file=...` | GET | 预览经过类型、大小和敏感信息检查的文本文件 |",
        "| `/api/skills/export?path=...` | GET | 导出自动排除凭据、密钥和缓存的 Skill ZIP |",
        "| `/api/skills/import` | POST | 校验 ZIP、拒绝危险文件并导入到指定来源和功能分类 |",
        "| `/api/skills/analyze` | POST | 调用本机 Codex 静态分析 Skill 并持久化六类结论 |",
        "| `/api/skills/move` | POST | 更新自创/已测/待测维护标签并移动目录及关联元数据 |",
        "| `/api/skills/delete` | POST | 将 Skill 移入本地 `.skill-trash` 回收目录 |",
    ...
    lines.extend([
        ...
        "- 修改 Playground 后必须重启，并检查 `/api/config`、`/api/skills`、页面模块加载、分类切换、详情/文件浏览和对话发送按钮。",
        "",
    ])
    return "\n".join(lines)
